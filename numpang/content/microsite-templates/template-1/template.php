...
function appendMicrositeUtmParams($url) {
    ...
    return $url . (strpos($url, '?') === false ? '?' : '&') . http_build_query($utmParams);
}
...
function getMicrositeIpAddress() {
...
function getMicrositeDeviceType() {
...
$userIp = getMicrositeIpAddress();
$deviceType = getMicrositeDeviceType();
...
function selectMicrositeRandomWithPercentage($items)
{
    $totalPercentage = 0;
...
if (!empty($microsite_pop_under)) {
    $randomData = selectMicrositeRandomWithPercentage($microsite_pop_under);
}
